...
(defn get-applicants
  ...
  (->> files
       ...
       csv/read-csv
       rest 
       (map #(let [applicant-id (Integer/parseInt (nth % 0))
                   application-date (local-date "yyyy-MM-dd" (nth % 1))
                   first-name (lower-case (nth % 2))
                   last-name (lower-case (nth % 3))
...
(defn get-employees [files]
  (->> files
       ...
       csv/read-csv
       rest
       (map #(let [employee-id (Integer/parseInt (nth % 0))
                   first-name (lower-case (nth % 2))
                   last-name (lower-case (nth % 3))
                   full-name (str first-name " " last-name)
                   hire-date (local-date "yyyy-MM-dd"
                                         (get % 1 "2000-12-31"))
                   term-date (local-date "yyyy-MM-dd"
                                         (or (not-empty (get % 4))
                                             "2021-03-31"))
...
(defn compare-names-two-ways [a-first-name a-last-name
                              b-first-name b-last-name]
  (max (avg (compare-two-names a-first-name b-first-name)
            (compare-two-names a-last-name b-last-name))
       (avg (compare-two-names a-first-name b-last-name)
            (compare-two-names a-last-name b-first-name))))
...
(defn match:employees->applicants [employees applicants]
  (for [employee employees]
    (let [direction "employee->applicant"
          applicant-cands-direct
          (filter
           #(= (:employee-full-name employee)
               (:applicant-full-name %))
           applicants)]
      (cond (<= 1 (count applicant-cands-direct))
            (merge {:direction direction
                    :matching-score 1.0}
                   employee 
                   (first applicant-cands-direct))
            :default
            (as-> (->> applicants 
                       (map
                        (fn [applicant]
                          (hash-map
                           ...
                           (compare-names-two-ways
                            ...
                            (:applicant-last-name applicant)))))
                       (sort-by (juxt :matching-score))
                       last) applicant-cand-eval-w-score
              (merge {:direction direction
                      :matching-score
                      (:matching-score applicant-cand-eval-w-score)}
                     employee
                     (:record applicant-cand-eval-w-score)))))))
